      ...
      if (response.status === 200) {
        if (data.count === 0) {
          ...
          for (let i = 0; i < data.tasks.length; i++) {
            ...
            let rowHTML = `<td>${data.tasks[i].toDo}</td><td>${data.tasks[i].description}</td><td>${data.tasks[i].status}</td>${editButton}${deleteButton}`;
    ...
    document.addEventListener("startDisplay", async () => {
      ...
      if (token) {
        ...
        if (count > 0) {
          ...
          tasksMessage.textContent = "There are no tasks to display for this user.";
        ...
        if (e.target === logoff) {
          ...
          // Delete
            ...
            try {
              ...
              const data = await response.json();
              if (response.status === 200) {
                message.textContent = "The task was deleted successfully";
                thisEvent = new Event("startDisplay");
                document.dispatchEvent(thisEvent);
              } else {
                // if the delete fails
                message.textContent = "Failed to delete the task. Try again later.";
                thisEvent = new Event("startDisplay");
                document.dispatchEvent(thisEvent);
              }
            } catch (err) {
